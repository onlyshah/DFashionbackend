/**
 * ============================================================================
 * VENDOR CONTROLLER - PostgreSQL/Sequelize Version
 * ============================================================================
...
exports.getAllVendors = async (req, res) => {
  try {
    ...
    const where = {};
    if (status && VENDOR_STATUSES.includes(status)) where.status = status;
    if (verified_only) where.is_verified = true;
    if (search) {
      where[Op.or] = [
...
exports.createVendor = async (req, res) => {
  try {
    ...
    const existing = await models.Vendor.findOne({ where: { user_id: req.user.id } });
    if (existing) return ApiResponse.error(res, 'You already have a vendor account', 409);

    const t = await models.sequelize.transaction();
    try {
      const vendor = await models.Vendor.create({
        user_id: req.user.id, shop_name, business_email, phone, address, city, state, postal_code, country,
        shop_description, bank_account_holder, bank_account_number, bank_ifsc,
        status: 'pending', is_verified: false, commission_rate: 10, rating: 0, total_sales: 0
      }, { transaction: t });

      await models.VendorVerification.create({
        vendor_id: vendor.id, status: 'pending', documents: {}
      }, { transaction: t });

      await t.commit();
      return ApiResponse.created(res, {
        vendor_id: vendor.id, shop_name: vendor.shop_name, status: vendor.status,
...
exports.updateVendor = async (req, res) => {
  try {
    ...
    const vendor = await models.Vendor.findByPk(vendor_id, { 
      include: [{ model: models.User, as: 'owner', attributes: ['id', 'username', 'email'] }] 
    });
    if (!vendor) return ApiResponse.notFound(res, 'Vendor');
...
exports.getVendorStats = async (req, res) => {
  try {
    ...
    const orders = await models.Order.count({
      include: [{ model: models.OrderItem, where: { vendor_id }, required: true }]
    });
    ...
    return ApiResponse.success(res, {
      vendor_id, shop_name: vendor.shop_name, products_count: products, orders_count: orders,
      total_revenue, commission_rate: vendor.commission_rate, rating: vendor.rating,
      status: vendor.status, is_verified: vendor.is_verified
...
exports.submitVerificationDocuments = async (req, res) => {
  try {
    ...
    const verification = await models.VendorVerification.findOne({ where: { vendor_id } });
    if (!verification) return ApiResponse.notFound(res, 'Verification record');

    const documents = {
      business_license: business_license_url, tax_id, tax_id_document: tax_id_url,
      owner_identity: owner_identity_url, additional: additional_docs || {}, submitted_at: new Date()
    };

    await verification.update({ documents, status: 'pending', submitted_at: new Date() });

    return ApiResponse.success(res, {
      verification_id: verification.id, status: 'pending',
...
exports.getVerificationStatus = async (req, res) => {
  try {
    ...
    return ApiResponse.success(res, {
      vendor_id, status: verification.status,
  ...
  try {
    ...
    try {
      await verification.update({
        status: 'approved', verified_at: new Date(), verified_by: req.user.id
      }, { transaction: t });

      await vendor.update({ status: 'active', is_verified: true }, { transaction: t });
      await t.commit();

      return ApiResponse.success(res, {
        vendor_id, status: 'active', is_verified: true, message: 'Vendor approved successfully'
...
exports.rejectVendor = async (req, res) => {
  try {
    ...
    await verification.update({
      status: 'rejected', rejection_reason: reason, rejected_at: new Date(), rejected_by: req.user.id
    });

    return ApiResponse.success(res, { vendor_id, status: 'rejected', reason }, 'Vendor rejected successfully');
  } catch (error) {
...
exports.suspendVendor = async (req, res) => {
  try {
    ...
    await vendor.update({
      status: 'suspended', suspension_reason: reason, suspended_at: new Date(), suspended_by: req.user.id
    });

    return ApiResponse.success(res, { vendor_id, status: 'suspended', reason }, 'Vendor suspended successfully');
  } catch (error) {
...
exports.getVendorDashboard = async (req, res) => {
  try {
    const vendor_id = req.params.vendor_id || req.user.vendor_id;
    const vendor = await models.Vendor.findByPk(vendor_id);
    if (!vendor) return ApiResponse.notFound(res, 'Vendor');

    return ApiResponse.success(res, vendor, 'Vendor dashboard retrieved');
  } catch (error) {
...
exports.getVendorOrders = async (req, res) => {
  try {
    ...
    const orders = await models.Order.findAll({
      include: [{ model: models.OrderItem, where: { vendor_id } }]
    });

    return ApiResponse.success(res, orders || [], 'Vendor orders retrieved');
  } catch (error) {
...
exports.getVendorEarnings = async (req, res) => {
  try {
    const { vendor_id } = req.params;
    const earnings = await models.VendorEarnings?.findOne({ where: { vendor_id } }) || {};

    return ApiResponse.success(res, earnings, 'Vendor earnings retrieved');
  } catch (error) {
...
exports.approvePayout = async (req, res) => {
  try {
    ...
    if (!amount || amount <= 0) return ApiResponse.error(res, 'Invalid payout amount', 422);
    ...
    try {
      const payout = await models.VendorPayout.create({
        vendor_id, amount, status: 'approved', approved_at: new Date(), approved_by: req.user.id
      }, { transaction: t });

      await vendor.update({ total_sales: vendor.total_sales - amount }, { transaction: t });
      await t.commit();

      return ApiResponse.success(res, {
        payout_id: payout.id, vendor_id, amount, status: 'approved'
