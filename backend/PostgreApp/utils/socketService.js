...
class SocketService {
  constructor() {
    ...
    this.models = null; // Will be loaded after database is ready
  }

  // Set models after they're ready
  setModels(models) {
    this.models = models;
  }
  ...
  setupMiddleware() {
    ...
    this.io.use(async (socket, next) => {
      try {
        if (!this.models) {
          return next(new Error('Models not initialized'));
        }
        ...
        const user = await this.models.User.findById(decoded.id).select('-password');
  ...
  handleNotificationEvents(socket) {
    ...
    socket.on('markNotificationRead', async (data) => {
      try {
        ...
        await this.models.Notification.markAsRead(notificationId, socket.userId);
    ...
    socket.on('markAllNotificationsRead', async () => {
      try {
        await this.models.Notification.markAllAsRead(socket.userId);
    ...
    socket.on('getNotifications', async (data) => {
      try {
        ...
        const result = await this.models.Notification.getUserNotifications(socket.userId, options);
  ...
  async sendUnreadCount(userId) {
    try {
      const count = await this.models.Notification.getUnreadCount(userId);
  ...
  async sendOrderNotification(userId, order, type) {
    ...
    const notification = await this.models.Notification.createNotification(notificationData);
